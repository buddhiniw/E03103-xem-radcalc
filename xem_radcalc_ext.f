...
C       at central
	...
	epmin=.8*epmin
	x_min=4*e1*epmin*0.80*(sin((deg*d_r-0.034)/2))**2/(2*m_p*(e1
	1 -epmin*0.80))
...
!	epmin = 0.88*epmin
	...
	read(5,*) a_nuc
         a_nuc = 27 ! do only aluminium 

	write(6,*) '**Target can?'
	read(5,*) string

	if(string.eq.'can') then
	   can_flag=.true.
	endif

	write(6,*) '**up or down stream window?'
	read(5,*) string

	if(string.eq.'dow') then
	   downstream=.true.
	else
	   downstream=.false.
	endif
	...
	epmax=x_max*e1*m_p/(2*e1*(sin((deg*d_r-0.034)/2))**2+m_p*x_max)

	if(epmax.ge.e1) epmax=e1-0.01
	write (6,*) '**epmin and epmax are', epmin,epmax, a_nuc
...
c	call target_info(theta_tar,deg,tb,ta,zp,a_nuc,tgt_len,tgt_thick,aux,teff)
	call target_info(can_flag,downstream,theta_tar,deg,tb,ta,zp,a_nuc,tgt_len,tgt_thick,aux,teff)
...
!	  aa = 1.07599187996174
!	  bb = -0.0902164446608597
!	  cc = 0.00307987969057965
!	  dd=0

	  aa = 1.36909582093995
	  bb = -0.397730607400245
	  cc = -0.00610340423044023
	  dd = 0.07861291737992
...
!	  dd = -0.0864993    
	  aa = 1.08000840429859
	  bb = -0.0982300330454544
	  cc = 0.000586613632274299
	  dd = 0.0316207297635641
	elseif(a_nuc.eq.9) then
	  m_atom=9.012182
!	  aa = 0.844377382146694
!	  bb = 0.0876207985614605
!	  cc = -8.33623766654011e-05
!	  dd = 0.0318823596102534
	  aa = 1.025
	  bb = -0.0287152887511119
	  cc = 0.00445485764339834
	  dd = 0.0!032096131872732
...
!	  dd  = 0.0591535   
	  aa = 1.27376778137511
	  bb = -0.331919290386685
	  cc = -0.000807971358556216
	  dd = 0.0698843279899779
