	real*8 function radiate(ep_arg,modelhack)
...
C Declare locals.
	...
	real*8  b,tot,fac,btr,ta,tb,sig,cross
	real*8  dlz,eta
...
C Function definitions.
	...
	external funca,funcb,quadmo
	real*8  funca,funcb,quadmo
	real*8  ddilog
...
C Soft photon piece.
	...
	sig = 0.D00
	call sigmodel(es,ep,th,zp+zn,zp,tm,aux,sig,modelhack)
	mmsq = m_p*m_p + 2.*m_p*nu - q2
	call model_fix(-mmsq,sig)
	cross = sig

 	radiate = (r*dee/es)**btb*(dee/ep)**bta*fac*cross*
     >	(1.d0 - xi/((1.d0 - (btb + bta))*max(dee,10.D00*xi))) 

C Add in integrals. (The real CPU sink!)

	radiate = radiate + quadmo(funca,esmin,esmax,1.d-3,idum,modelhack) +
     >	quadmo(funcb,epmin,epmax,1.d-3,idum,modelhack)

	return 

	entry radiate_init(e_bm,th_scat,t_b,t_a,z_p,z_n,m_nuc,aux_arg,de_e)
C+______________________________________________________________________________
!
...
C-______________________________________________________________________________
	...
	real*8  es1
	real*8  sig			!Model cross section.
...
C Put it together.
	...
	nu = es1 - ep
	sig = 0.D00
	call sigmodel(es1,ep,th,zp+zn,zp,tm,aux,sig,modelhack)
...
C Calculate SIGMODEL.
	   ...
	   call sigmodel(es,ep1,th,zp+zn,zp,tm,aux,sig,modelhack)
	   nu = es - ep1
	   q2 = 4.D0*es*ep1*snsq
	   mmsq = m_p*m_p + 2.*m_p*nu - q2
