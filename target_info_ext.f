	subroutine target_info(can_flag,downstream,theta_tar,theta,tb,ta,z,a,tgt_len,tgt_thick,aux,teff)
...
C Get target specific stuff from lookup table.
	...
	if(can_flag) then !al walls of cryo target
	   tgt_thick = 0.035 !g/cm2 only one wall!
	else
	   tgt_thick = 0.5259/2.0 !only one slab!!
	endif
...
C In this subroutine, always calling for aluminum.
C If "can_flag", then add ~0.5% radiation length from cryotarget.
	   if(can_flag) then
	      if(downstream) then
		 tb = tgt_rl+tgt_rl/2. + 0.00494625d0
		 ta = tgt_rl/2./abs(cos(thrad+spect_sign*thrad_tar))
	      else
		 tb = tgt_rl/2.0
C Path through cryogen only
		 radius = 1.955d
		 xtar = 2.0d0*radius*tan(thrad)/(1.0+tan(thrad)**2)
		 ytar = -radius + xtar*tan(thrad)
		 pathlen1 = sqrt((ytar+radius)**2+xtar**2)
C Path through cryogen + wall
		 radius2 = radius + 0.0125d0
		 xtar = (radius*tan(thrad) +
	1	      sqrt(radius2**2*(1.0+tan(thrad)**2)-radius**2))/(1.0+tan(thrad)**2)
		 ytar = -radius + xtar*tan(thrad)
		 pathlen2 = sqrt((ytar+radius)**2+xtar**2)
		 dpath = pathlen2-pathlen1
		 ta = tgt_rl/2./abs(cos(thrad+spect_sign*thrad_tar))+ 
	1	      dpath/0.0125d0*tgt_rl + pathlen1/(2.*radius) * 0.00494625d0
	      endif		 
	   else
	      if(downstream) then
		 tb = tgt_rl+tgt_rl/2.d0
		 ta = tgt_rl/2.d0/abs(cos(thrad+spect_sign*thrad_tar))
	      else
		 tb = tgt_rl/2.d0
C Here I'm checking if the scattered electron goes through the downstream
C foil of the dummy target
		 dx = 3.935d0*tan(thrad) ! dummy foils 3.935 cm apart (warm)
		 xlim = 1.125d0*2.54d0     ! dummy is +/-1 inch, but we were about 1/8 inch to the left
		 if(dx.le.xlim) then !it goes through downstream foil
		    ta = 3.0d0*tgt_rl/2.d0/abs(cos(thrad+spect_sign*thrad_tar))
		 else                !misses downtream foil
		    ta = tgt_rl/2.d0/abs(cos(thrad+spect_sign*thrad_tar))
		 endif
	      endif
	   endif
